...
unet.compile(optimizer = opt, loss = dice_loss, metrics = [dice_metric, iou,'acc'])

history = unet.fit(train_gen,epochs = 100, steps_per_epoch=len(train_dateset)/8, validation_data = test_gen, validation_steps= len(test_dataset)/8, callbacks = [checkpointer, earlystopping, reduce_lr])
